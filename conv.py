...
class myGATConv_plus(nn.Module):
    """
    Adapted from
    ...
    def __init__(self,
                 ...
                 bias=False,
                 alpha=0.0):
        super(myGATConv_plus, self).__init__()
    ...
    def forward(self, graph, feat, e_feat, res_attn=None):
        with graph.local_scope():
            ...
            graph.apply_edges(fn.u_add_v('el', 'er', 'e'))  # dgl.fn
            # e = self.leaky_relu(graph.edata.pop('e') + graph.edata.pop('ee'))
            e = self.leaky_relu(graph.edata.pop('e'))
            # compute softmax
            graph.edata['a'] = self.attn_drop(edge_softmax(graph, e))

            # graph.edata['a'] = graph.edata['a'] + graph.edata['w'].unsqueeze(-1).repeat(1, self._num_heads).unsqueeze(
            #     -1)
