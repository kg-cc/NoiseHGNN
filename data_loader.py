...
class data_loader:
    def __init__(self, path, args):
        ...
        # self.links = self.load_links(args)
        self.links = self.load_links_with_wrong(args)
    ...
    def load_links_with_wrong(self, args):
        """
        return links dict
        ...
        with open(os.path.join(self.path, 'link_0.3_error.dat'), 'r', encoding='utf-8') as f:  # DBLP
            for line in f:
                th = line.split('\t')
    ...
    def load_links_generate_save_wrong(self, args):
        """
        return links dict
            total: total number of links
            count: a dict of int, number of links for each type
        ...
        with open(os.path.join(self.path, 'link.dat'), 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                # # todo  args.edge_wrong
                if random.random() < args.edge_wrong:
                    t_type = self.get_node_type(t_id)
                    start, end = node_range[t_type]
                    new_t_id = random.randint(start, end)  # Choose a new target node randomly
                    t_id = new_t_id
                ...
                links['count'][r_id] += 1
                links['total'] += 1
                processed_lines.append(f"{h_id}\t{t_id}\t{r_id}\t{link_weight}\n")
        with open(os.path.join(self.path, f'link_wrong_{args.edge_wrong}_{args.lct}.dat'), 'w', encoding='utf-8') as f:
    ...
    def load_links(self, args):
        ...
        node_range = self.node_range
        links = {'total': 0, 'count': Counter(), 'meta': {}, 'data': defaultdict(list)}
        with open(os.path.join(self.path, 'link_original.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                h_id, t_id, r_id, link_weight = int(th[0]), int(th[1]), int(th[2]), float(th[3])
                if r_id not in links['meta']:
    ...
    def load_nodes(self):
        ...
        node_range = {}
        last = shift.__len__()
        for i in range(shift.__len__() - 1):
            node_range[i] = (shift[i], shift[i + 1] - 1)
        node_range[last - 1] = (shift[last - 1], shift[last - 1] + nodes['count'][last - 1] - 1)
